  ...
  return (
    <header
      className={`fixed h-fit w-full laptop:w-fit z-40 right-0 p-3 py-6 transition-colors duration-1000 laptop:px-6 flex gap-10 ${
        isMobile && isScrolled
          ? "bg-warm-white/90 backdrop-blur border-t-2 bottom-0 "
          : isMobile ? "hidden"
          : isScrolled
            ? "bg-light-purple/100"
            : "bg-light-purple/0"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed top-0 right-0 h-screen w-screen overflow-hidden text-4xl bg-light-purple flex flex-col-reverse laptop:flex-col gap-8 items-end pr-6 pb-24 laptop:pb-0 laptop:pr-3 laptop:pt-8"
            ref={dropdownRef}
          >
            <RxCross1
